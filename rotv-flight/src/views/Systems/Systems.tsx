  ...
  return (
    <div className="systems-view">
      <header className="view-header systems-view__header">
        <div>
          ...
          <p className="view-subtitle">
            Connect and select at least one system to proceed to Pre-flight checks
          </p>
        </div>
        <div className="systems-view__header-meta">
          <span className="systems-view__count">
            {connectedSystems.length} connected · {selectedSystems.length} selected
          </span>
        </div>
      </header>
      ...
      {selectedSystems.length > 0 && (
        <section className="systems-view__selection-summary" aria-live="polite" aria-label="Selected systems">
          <span className="systems-view__selection-label">Selected for mission:</span>
          <ul className="systems-view__selection-list" role="list">
            {selectedSystems.map((s) => (
              <li key={s.id} className="systems-view__selection-tag">{s.name}</li>
            ))}
          </ul>
        </section>
      )}
